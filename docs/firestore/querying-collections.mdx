...
`useFirestoreInfiniteQueryData` hooks. The hooks both wrap the [`useInfiniteQuery`](https://react-query.tanstack.com/reference/useInfiniteQuery)
hook.
...
This also works with TypeScript, either by explicially providing the ID type or
inferred by the option:
...
// Explicit types
const products = useFirestoreQueryData<"_id", Product>("products", query, {
  idField: "_id",
});

// Inferred types
const query = collection(firebase, 'products').withConverter<Product>(...);
const products = useFirestoreQueryData("products", query, {
  idField: "_id",
});
```

In this example, the `Product` type will have the `_id` field joined (`Product & { _id: string }`).
